...
def build_parse_tree(expr):
    tokens = expr.split()
    my_tree = BinaryTree2()
    my_tree.set_root(TreeNode(""))
    current = my_tree.get_root()

    for token in tokens:
        if token == "(":
            current = current.insert_left(TreeNode("", current))
        elif token in {"+", "-", "/", "*"}:
            current.set_val(token)
            current = current.insert_right(TreeNode("", current))
        elif token == ")":
...
def main():
    ...
    # myTree = BinaryTree2()
    # myTree.set_root(TreeNode(10))
    # myTree.get_root().insert_left(TreeNode(5))
    # myTree.get_root().insert_right(TreeNode(15))
    # myTree.get_root().get_left_child().insert_left(TreeNode(2))
    # myTree.get_root().get_left_child().insert_right(TreeNode(7))
    # myTree.get_root().get_right_child().insert_left(TreeNode(12))
    # myTree.get_root().get_right_child().insert_right(TreeNode(17))
    #
    # print(myTree)

    new_tree = build_parse_tree("( 5 + 4) + ( ( 3 * 2) + ( 8 / 4 ) ) - 5")
    print(new_tree)
